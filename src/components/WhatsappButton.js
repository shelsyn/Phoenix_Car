...
export default function WhatsappButton() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
      {/* Nube de mensaje */}
      <div className={`transition-all duration-500 ease-in-out transform ${isOpen ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-4 scale-95"} ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
        <div className="bg-white shadow-lg rounded-lg p-4 w-72">
          ...
          <p className="text-sm text-gray-700 mb-4">Contáctanos para conocer todos los beneficios que ofrecemos. ¡Estamos aquí para ayudarte!</p>
          <a
      ...
      <button
        onClick={toggleChat}
        className={`flex items-center justify-center w-14 h-14 bg-green-500 rounded-full shadow-lg hover:shadow-xl hover:bg-green-600 focus:outline-none transition-transform duration-300 ${isOpen ? "rotate-45" : "rotate-0"}`}
      >
